...
public class OutputLexico2 {
    ...
    public OutputLexico2(String nombreFichero) {
    ...
    public void imprimirTablaEstatica() {
        String cad ="Tablerinha Estatinha (/n_n)/\n";
        Lexema lexema;
        for (int i = 0; i < instrucciones.size(); i++) {
            lexema = (Lexema) instrucciones.get(i);
            if(!lexema.componenteLexico.equals("identificador"))
                cad+="["+lexema.renglon+"]["+lexema.columna+"]\t\t\t"+ lexema.lexema + "\t\t\t"+lexema.componenteLexico+"\n";
        }
